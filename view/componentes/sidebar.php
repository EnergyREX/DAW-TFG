...
function renderizarSidebar($rol) {
  switch($rol) {
    case "paciente":
      sidebarPaciente($_SESSION['email'], $_SESSION['especialidad']);
      break;
    case "asistente":
      sidebarAdmin($_SESSION['email'], $_SESSION['especialidad']);
      break;
    case "doctores":
      sidebarAdmin($_SESSION['email'], $_SESSION['especialidad']);
      break;
...
function sidebarAdmin($email, $especialidad) {
  echo "
  <nav>
    <ul class='sidebar'>
      <li>
        <div class='user__info'>
          ...
          <h2 class='user__name'>$email</h2>
          <h4 class='user__role'>$especialidad</h4>
...
function sidebarPaciente($email, $especialidad) {
  echo "
  <nav>
    <ul class='sidebar'>
      <li>
        <div class='user__info'>
          <img class='user__pfp' src='./public/admin.jpg'>
          <h2 class='user__name'>$email</h2>
          <h4 class='user__role'>$especialidad</h4>
          <button class='user__btn'>View Profile</button>
        </div>
